...
locals {
  ...
  processed_adbs = {
    for adb_key, adb in local.adbs_config : adb_key => merge(adb, {
      ...
      compartment_id = adb.compartment_id != null ? (
      ...
      kms_key_id = adb.kms_key_id != null ? (
      ...
      # Display name
      display_name = adb.display_name != null ? adb.display_name : adb.db_name
    })
  }
}
